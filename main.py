...
import sys
import os
from features import cnt_features
...
class ExampleHook(tf.train.SessionRunHook):
    ...
    def before_run(self, run_context):
        return tf.estimator.SessionRunArgs(
            [tf.train.get_global_step(), self.item])
...
if args.model == "DCN":
    model = DCN("DCN", model_dir=model_dir, embedding_upload_hook=new_hook, high_param=high_param)
...
# feature_description.update({
#     f: tf.io.FixedLenFeature([3], tf.int64)
#     for f in list_features
# })
